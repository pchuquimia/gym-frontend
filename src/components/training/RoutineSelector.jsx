  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full rounded-2xl border border-[color:var(--border)] bg-[color:var(--card)] shadow-sm p-4 text-left hover:border-blue-200 transition-colors"
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-blue-600">{routine.location}</p>
            ...
            <div className="space-y-1 max-h-60 overflow-y-auto">
              {routines.map((r) => (
                <button
                  ...
                  className={`w-full rounded-xl px-3 py-2 text-left hover:bg-slate-100 dark:hover:bg-card ${
                    r.id === routine.id
                      ? "border border-blue-200 bg-blue-50 dark:bg-card"
                      : "border border-transparent"
                  }`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-xs font-semibold text-blue-600">{r.location}</p>
                      <p className="text-sm font-semibold text-[color:var(--text)]">{r.name}</p>
                      <p className="text-[12px] text-[color:var(--text-muted)]">
                        {r.exerciseCount} ejercicios | Ultimo: {r.lastDate}
                      </p>
                    </div>
                    {r.id === routine.id && <span className="text-blue-600 text-sm font-semibold">Elegido</span>}
                  </div>
                </button>
              ))}
            </div>
