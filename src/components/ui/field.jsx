...
export function Field({ label, hint, error, children, className = "", id }) {
  return (
    <label
      className={`ui-field flex min-w-0 flex-col gap-2 ${className}`}
      htmlFor={id}
    >
      {label ? (
        <span className="ui-field__label font-sans text-sm font-semibold text-[color:var(--text)]">
          {label}
        </span>
      ) : null}
      {children}
      {error ? (
        <span
          className="ui-field__error font-sans text-xs font-medium text-[color:var(--danger)]"
          role="alert"
        >
          {error}
        </span>
      ) : hint ? (
        <span className="ui-field__hint font-sans text-xs text-[color:var(--text-muted)]">
          {hint}
        </span>
      ) : null}
    </label>
