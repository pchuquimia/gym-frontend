...
function StatCard({ label, value, suffix, children, onClick }) {
...
function Dashboard({ onNavigate = () => {}, coachAthlete = null }) {
    ...
    const muscleReadiness = Array.from(muscleStats.values())
      .map((item) => {
        ...
        const value = Math.max(
          10,
          Math.min(100, Math.round(100 - fatigue * 48)),
        );
  ...
  return (
    <motion.div
      initial={false}
      className="dashboard-shell dashboard-pilot mx-auto w-full max-w-md space-y-6 px-0 pb-10 pt-0 text-[color:var(--text)] md:max-w-5xl md:space-y-4 md:px-0 md:pt-0 xl:max-w-6xl 2xl:max-w-[1280px]"
    >
      <MobileDailyPlan
        date={now}
        profile={profile}
                  ...
                  {weeklySets.byPrimaryMuscle.length ? (
                    <div className="mt-6 space-y-5">
                      {weeklySets.byPrimaryMuscle.map((item) => {
                        const maxSets =
                          weeklySets.byPrimaryMuscle[0]?.sets || 1;
