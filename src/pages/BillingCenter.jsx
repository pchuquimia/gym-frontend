        ...
        <div className="grid gap-3 md:grid-cols-2">
          {plans.map((plan) => {
            ...
            return (
              <article
                key={plan.id}
                className={`relative overflow-hidden border bg-[color:var(--card)] p-5 ${isCurrent ? "border-[#181918] dark:border-[#e2ff00]" : "border-[color:var(--border)]"}`}
              >
                {isRecommended ? (
                  <span className="absolute right-0 top-0 bg-[#181918] px-3 py-1.5 text-[9px] font-black uppercase text-white dark:bg-[#e2ff00] dark:text-black">
                    Recomendado
                  </span>
                ) : null}
