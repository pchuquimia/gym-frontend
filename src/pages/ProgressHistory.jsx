...
import { formatList, toArray } from "../constants/exerciseTaxonomy";
...
function ProgressHistory() {
  const { exercises, sessions, trainings, updateExerciseMeta } =
    useTrainingData();
    ...
    return ids
      .map((id) => {
        ...
        return {
          ...
          equipment: formatList(meta?.equipment, ""),
        };
      })
      .filter(Boolean);
  }, [exercises, sessions]);
  ...
  const chartPoints = useMemo(() => {
    ...
    return filteredSessions.map((s) => {
      ...
      return {
        date: s.date,
        peso: bestWeightSet,
        duracion_ejercicio: durEj,
        duracion_sesion: durSes,
        isPr,
      };
    });
  }, [filteredSessions]);
  ...
  const saveEdit = () => {
    updateExerciseMeta(selectedExercise.id, {
      ...editForm,
  ...
  return (
    <>
            ...
            <ResponsiveLine
              data={Object.values(
                filteredTrainings.reduce((acc, t) => {
                  ...
                  if (!acc[key])
                    acc[key] = { id: key, color: "#4fa3ff", data: [] };
                  acc[key].data.push({
                    x: t.date,
                    y: (t.durationSeconds || 0) / 60,
                  });
                  return acc;
                }, {}),
              )}
              ...
              tooltip={({ point }) => (
                <div className="text-xs bg-[#0d1a2b] border border-border-soft rounded-md px-3 py-2 text-white shadow-lg">
                  ...
                  <div className="text-muted">
                    {new Date(point.data.x).toLocaleDateString("es-ES", {
                      day: "2-digit",
                      month: "short",
                    })}
                  </div>
      ...
      <section className="grid gap-3 md:grid-cols-3">
        <div
          className={`rounded-2xl border border-border-soft px-4 py-3 text-white shadow-soft ${getStatusColor("up")}`}
        >
          <p className="font-semibold">Mejora</p>
          <p className="text-sm text-emerald-200">
            Tu rendimiento general ha aumentado.
          </p>
        </div>
        <div
          className={`rounded-2xl border border-border-soft px-4 py-3 text-white shadow-soft ${getStatusColor("stable")}`}
        >
        ...
        <div
          className={`rounded-2xl border border-border-soft px-4 py-3 text-white shadow-soft ${getStatusColor("down")}`}
        >
      ...
      <section className="card space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span
            className={`text-xs px-3 py-1 rounded-full border ${status === "up" ? "border-emerald-400 text-emerald-200" : status === "down" ? "border-rose-400 text-rose-200" : "border-amber-400 text-amber-200"}`}
          >
      ...
      <section className="card space-y-3">
        <h3 className="text-lg font-semibold">
          Historial Detallado: {selectedExercise?.name || ""}
        </h3>
              ...
              {filteredSessions.length === 0 && (
                <tr>
                  <td className="py-3 text-muted" colSpan={4}>
                    No hay registros para este ejercicio en el rango
                    seleccionado.
                  </td>
                </tr>
              )}
              {filteredSessions.map((row) =>
                row.sets.map((set, idx) => (
                  ...
                  >
                    ...
                    <td>{idx + 1}</td>
                    <td>{set.reps}</td>
                    <td className={set.weight < 78 ? "text-accent-red" : ""}>
                      {set.weight}
                    </td>
                  </tr>
                )),
