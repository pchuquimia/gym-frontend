...
export default function CoachManagement({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    const closeMenu = (event) => {
      ...
      if (event.type === "pointerdown" && event.target.closest("[data-user-actions]")) return;
      setOpenUserMenuId("");
    };
  ...
  const coachRows = useMemo(
    () =>
      coachAccounts
        ...
        .sort((a, b) => b.athleteCount - a.athleteCount || a.name.localeCompare(b.name)),
  ...
  const updateUser = async (target, payload, successMessage) => {
    ...
    try {
      ...
      setUsers((current) =>
        current.map((item) => (idOf(item) === targetId ? { ...item, ...updated } : item)),
      );
  ...
  return (
    <main className="management-shell mx-auto w-full max-w-[1280px] pb-24 text-[color:var(--text)]">
      <header className="grid gap-4 border-b border-[color:var(--border)] pb-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-[13px] font-semibold leading-5 text-[color:var(--text-muted)]">
            Supervisa adopcion, distribucion y acceso. Cada coach administra su propia cartera.
          </p>
        </div>
        <button
          type="button"
      ...
      <nav className="mt-4 grid grid-cols-2 border border-[color:var(--border)] bg-[color:var(--card)] p-1 md:w-fit" aria-label="Vistas de administracion">
        {[
          ["overview", "Resumen"],
      ...
      {loading ? (
        ...
        <div className="mt-4 space-y-6">
          <section className="grid grid-cols-2 gap-2 xl:grid-cols-4" aria-label="Indicadores principales">
            {[
              ["Atletas", athletes.length, Users, `${athletes.length - linkedAthletes.length} independientes`],
              ["Con coach", linkedAthletes.length, UserRoundCheck, `${linkedPercent}% de los atletas`],
              ["Cuentas coach", coachAccounts.length, ShieldCheck, `${activeCoachAccounts.length} activas, incluye admins`],
              ["Promedio", averageLoad, Gauge, "atletas por coach activo"],
            ].map(([label, value, Icon, detail]) => (
              <article key={label} className="border border-[color:var(--border)] bg-[color:var(--card)] p-4 shadow-sm dark:shadow-none">
          ...
          <section>
            <div className="mb-2 flex items-end justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase text-[#ff5722] dark:text-[#e2ff00]">Operacion</p>
                <h2 className="mt-1 text-xl font-black uppercase">Distribucion por coach</h2>
              </div>
              <span className="text-[11px] font-bold text-[color:var(--text-muted)]">{linkedAthletes.length} vinculados</span>
            </div>
            <div className="border border-[color:var(--border)] bg-[color:var(--card)]">
              <div className="hidden grid-cols-[minmax(220px,1fr)_120px_minmax(160px,0.8fr)_100px] gap-4 border-b border-[color:var(--border)] bg-[color:var(--bg)] px-4 py-2 text-[10px] font-black uppercase text-[color:var(--text-muted)] md:grid">
                <span>Responsable</span><span>Rol</span><span>Carga relativa</span><span>Atletas</span>
              </div>
              <div className="divide-y divide-[color:var(--border)]">
                {coachRows.map((coach) => {
                  const coachId = idOf(coach);
                  const maxLoad = Math.max(1, ...coachRows.map((item) => item.athleteCount));
                  const loadPercent = Math.round((coach.athleteCount / maxLoad) * 100);
                  return (
                    <div key={coachId} className="grid gap-3 px-4 py-3 md:grid-cols-[minmax(220px,1fr)_120px_minmax(160px,0.8fr)_100px] md:items-center md:gap-4">
                      <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <p className="truncate text-[14px] font-black">{coach.name}{coachId === currentAdminId ? " · Tu cuenta" : ""}</p>
                          <p className="truncate text-[11px] font-semibold text-[color:var(--text-muted)]">{coach.email}</p>
                        </div>
                      </div>
                      <span className="w-fit border border-[color:var(--border)] px-2 py-1 text-[9px] font-black uppercase">{roleLabel(coach.role)}</span>
                ...
                {!coachRows.length ? <p className="px-4 py-10 text-center text-sm text-[color:var(--text-muted)]">Todavia no hay cuentas coach.</p> : null}
              </div>
            </div>
          </section>

          <section className="border-l-2 border-[#ff5722] bg-[#fff5f1] p-4 dark:border-[#e2ff00] dark:bg-[#171900]">
            <p className="text-[11px] font-black uppercase">Vinculacion sin intermediarios</p>
            <p className="mt-1 max-w-3xl text-[12px] font-semibold leading-5 text-[color:var(--text-muted)]">
              En beta, toda cuenta nueva inicia como atleta independiente. El coach comparte su codigo y el atleta decide vincularse desde Perfil. El administrador solo supervisa los indicadores generales.
            </p>
          </section>
        </div>
      ) : (
        <section className="mt-4">
          <div className="grid gap-3 border border-[color:var(--border)] bg-[color:var(--card)] p-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center">
            ...
            <p className="text-[11px] font-bold text-[color:var(--text-muted)]">{filteredUsers.length} de {users.length} cuentas</p>
          </div>
          <div className="mt-2 border border-[color:var(--border)] bg-[color:var(--card)]">
            <div className="hidden grid-cols-[minmax(260px,1fr)_130px_170px_90px_auto] gap-4 border-b border-[color:var(--border)] bg-[color:var(--bg)] px-4 py-2 text-[10px] font-black uppercase text-[color:var(--text-muted)] md:grid">
              <span>Cuenta</span><span>Rol</span><span>Relacion</span><span>Estado</span><span>Mas</span>
            </div>
            <div className="divide-y divide-[color:var(--border)]">
              {filteredUsers.map((item) => {
                const itemId = idOf(item);
                const isCurrentAdmin = itemId === currentAdminId;
                const linked = item.role === "Cliente" && item.assignedTrainerId;
                return (
                  <div key={itemId} className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 px-3 py-3 md:grid-cols-[minmax(260px,1fr)_130px_170px_90px_auto] md:px-4">
                    <div className="flex min-w-0 items-center gap-3">
                      <ProfileAvatar
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="truncate text-[11px] font-semibold text-[color:var(--text-muted)]">{item.email}</p>
                      </div>
                    </div>
                    <span className="justify-self-end text-[10px] font-black uppercase md:justify-self-start">{roleLabel(item.role)}</span>
                    ...
                    <div className="justify-self-end" data-user-actions>
                      {isCurrentAdmin ? (
                        ...
                        <div className="relative">
                          <button type="button" onClick={() => openActions(itemId)} className="grid h-9 w-9 place-items-center border border-[color:var(--border)]" aria-label={`Acciones para ${item.name}`}>
                            {savingId === itemId ? <Loader2 className="h-4 w-4 animate-spin" /> : <MoreVertical className="h-4 w-4" />}
                          </button>
                            ...
                            <div className="absolute right-0 top-[calc(100%+0.35rem)] z-30 w-56 border border-[color:var(--border)] bg-[color:var(--card)] p-1.5 shadow-2xl">
                              {item.role !== "Admin" ? (
                                <button
                                  ...
                                  onClick={() => {
                                    setOpenUserMenuId("");
                                    setRoleChangeTarget({ user: item, role: item.role === "Entrenador" ? "Cliente" : "Entrenador" });
                                  }}
                                >
                                  <UserCog className="h-4 w-4" />
                                  {item.role === "Entrenador" ? "Convertir en atleta" : "Dar acceso de coach"}
                                </button>
                              ) : null}
                              <button
                                type="button"
              ...
              {!filteredUsers.length ? <p className="px-4 py-10 text-center text-sm text-[color:var(--text-muted)]">No hay resultados para esta busqueda.</p> : null}
            </div>
          </div>
        </section>
      )}
      ...
      {roleChangeTarget ? (
        <div className="fixed inset-0 z-[90] flex items-end bg-black/60 sm:items-center sm:justify-center sm:p-4">
          <div className="w-full rounded-t-xl border border-[color:var(--border)] bg-[color:var(--card)] p-4 shadow-2xl sm:max-w-md sm:rounded-[4px]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase text-[#ff5722] dark:text-[#e2ff00]">Cambio de acceso</p>
                <h2 className="mt-1 text-xl font-black uppercase">{roleChangeTarget.role === "Entrenador" ? "Dar acceso de coach" : "Convertir en atleta"}</h2>
              </div>
              <button type="button" onClick={() => setRoleChangeTarget(null)} className="grid h-10 w-10 place-items-center border border-[color:var(--border)]" aria-label="Cerrar"><X className="h-4 w-4" /></button>
            </div>
            ...
            <div className="mt-5 grid grid-cols-2 gap-2">
              <Button variant="outline" disabled={Boolean(savingId)} onClick={() => setRoleChangeTarget(null)}>Cancelar</Button>
              <button type="button" onClick={confirmRoleChange} disabled={Boolean(savingId)} className="theme-accent-solid h-11 px-4 text-sm font-black disabled:opacity-50">Confirmar</button>
            </div>
          </div>
        </div>
      ) : null}
      ...
      {userToDelete ? (
        <div className="fixed inset-0 z-[90] flex items-end bg-black/60 sm:items-center sm:justify-center sm:p-4">
          <div className="w-full rounded-t-xl border border-[color:var(--border)] bg-[color:var(--card)] p-4 shadow-2xl sm:max-w-md sm:rounded-[4px]">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="text-[10px] font-black uppercase text-red-500">Accion permanente</p>
                <h2 className="mt-1 text-xl font-black uppercase">Eliminar usuario</h2>
              </div>
              <button type="button" onClick={() => setUserToDelete(null)} className="grid h-10 w-10 place-items-center border border-[color:var(--border)]" aria-label="Cerrar"><X className="h-4 w-4" /></button>
            </div>
            <p className="mt-4 text-[13px] font-semibold leading-5 text-[color:var(--text-muted)]">Se eliminaran la cuenta, entrenamientos, rutinas, fotos y ejercicios personalizados de <strong className="text-[color:var(--text)]">{userToDelete.name}</strong>.</p>
            <div className="mt-5 grid grid-cols-2 gap-2">
              <Button variant="outline" disabled={Boolean(savingId)} onClick={() => setUserToDelete(null)}>Cancelar</Button>
              <button type="button" onClick={deleteUser} disabled={Boolean(savingId)} className="h-11 bg-red-600 px-4 text-sm font-black text-white disabled:opacity-50">{savingId ? "Eliminando..." : "Eliminar"}</button>
            </div>
          </div>
        </div>
      ) : null}
